...
function getRelationArray() {
	...
	return (0, _relation.initPipeline)(importTreeJson, outputTreeJson, visibleInput, visibleOutput);
}
...
function initView(importTree, outputTree, linePathData) {
	...
	$("#removeLine").click(function () {
		...
		linePathData.relation = (0, _relation.delRelation)(linePathData.relation, fromPath);
		relationLineInit(linePathData.relation);
		$(this).addClass("hide");
	});
}
...
function relationLine(ary) {
	...
	for (var i = 0; i < ary.length; i++) {
		...
		if (fromDom.is(":visible") && toDom.is(":visible")) {
			settingOut([fromDom.offset().left, fromDom.offset().top, toDom.offset().left, toDom.offset().top], fromPath, toPath);
		}
		...
		if (fromDom.is(":visible") && toDom.is(":hidden")) {
			...
			if (parentDom != undefined) {
				settingOut([fromDom.offset().left, fromDom.offset().top, parentDom.offset().left, parentDom.offset().top], fromPath, toPath);
			}
		}
		...
		if (fromDom.is(":hidden") && toDom.is(":visible")) {
			...
			if (parentDom != undefined) {
				settingOut([parentDom.offset().left, parentDom.offset().top, toDom.offset().left, toDom.offset().top], fromPath, toPath);
			}
		}
...
function settingOut(point, fromPath, toPath) {
	...
	d3.select("#bipatiteLineSvg").append("path").attr("d", d).attr("stroke", "green").attr("stroke-width", 3).attr("fill", "none").attr("stroke-opacity", "0.8").attr("class", "cursor").attr("from", fromPath).attr("to", toPath).on("click", function (d, i) {
